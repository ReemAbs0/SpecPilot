...
export function SpecificationCard({ spec }: { spec: SavedSpecificationSummary }) {
  ...
  return (
    <Link
      to={`/library/${spec.id}`}
      className="group flex items-center gap-4 rounded-2xl border border-slate-100 bg-white p-5 shadow-card transition-colors hover:border-brand-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2"
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-3">
          <h2 className="truncate font-semibold text-slate-900">{spec.title}</h2>
          {created && <span className="shrink-0 text-xs text-slate-400">{created}</span>}
        </div>
        <p className="mt-0.5 truncate text-sm text-slate-500">{spec.idea}</p>
      </div>
